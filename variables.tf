...
variable "clusters" {
  ...
  type = list(
    object({
      ...
      attach_ibm_managed_security_group   = optional(bool)                                                                                                                     # Whether to attach the IBM-defined default security group (whose name is kube-<clusterid>) to all worker nodes. Only applicable if custom_security_group_ids is set.
      ...
      ignore_worker_pool_size_changes     = optional(bool)                                                                                                                     # Enable if using worker autoscaling. Stops Terraform managing worker count
      cluster_ready_when                  = optional(string)                                                                                                                   # The cluster is ready when one of the following: MasterNodeReady (not recommended), OneWorkerNodeReady, Normal, IngressReady
      enable_registry_storage             = optional(bool)                                                                                                                     # Set to `true` to enable IBM Cloud Object Storage for the Red Hat OpenShift internal image registry. Set to `false` only for new cluster deployments in an account that is allowlisted for this feature.
      ...
      worker_pools = optional(
        list(
          object({
            ...
            secondary_storage             = optional(string)      # Secondary storage type
            boot_volume_crk_name          = optional(string)      # Boot volume encryption key name
            operating_system              = optional(string)      # The operating system of the workers in the worker pool. If no value is specified, the current default version OS will be used. See https://cloud.ibm.com/docs/openshift?topic=openshift-openshift_versions#openshift_versions_available .
