...
| <a name="input_existing_vpc_cbr_zone_id"></a> [existing\_vpc\_cbr\_zone\_id](#input\_existing\_vpc\_cbr\_zone\_id) | Existing VPC CBR zone id. This is to open traffic flow only from landing zone VPCs to specific cloud services for an existing CBR rule that has context (network zones) set to VPC network zone. | `string` | `null` | no |
